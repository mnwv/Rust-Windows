...
fn main() {
    unsafe {
        ...
        let hwnd = CreateWindowExW(
            0,
            app_name,
            w!("Checker2 Mouse Hit-Test Demo"),
            WS_OVERLAPPEDWINDOW,
            CW_USEDEFAULT,
            CW_USEDEFAULT,
            CW_USEDEFAULT,
    ...
    unsafe {
        match message {
            ...
            WM_KEYDOWN => {
                ...
                let mut x = std::cmp::max(0, std::cmp::min(DIVISIONS as i32 - 1, point.x / CX_BLOCK));
                let mut y = std::cmp::max(0, std::cmp::min(DIVISIONS as i32 - 1, point.y / CY_BLOCK));
                ...
                match wparam as u16 {
                    ...
                    VK_RETURN | VK_SPACE => {
                        SendMessageW(window, WM_LBUTTONDOWN, MK_LBUTTON as usize, makelong!(x * CX_BLOCK, y * CY_BLOCK) as isize);
                    },
                    _ => {},
                }
                ...
                let mut point:POINT = POINT { x: x * CX_BLOCK + CX_BLOCK / 2, y: y * CY_BLOCK + CY_BLOCK / 2,};
