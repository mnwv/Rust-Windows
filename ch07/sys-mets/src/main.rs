...
static mut DELTA_PER_LINE: i32 = 0;
static mut ACCUM_DELTA: i32 = 0;
...
fn set_wheel_delta() {
    unsafe {
        let mut scroll_lines: u32 = 0;
        SystemParametersInfoW(SPI_GETWHEELSCROLLLINES, 0, &mut scroll_lines as *mut u32 as *mut core::ffi::c_void, 0);
        if scroll_lines != 0 {
            DELTA_PER_LINE = (WHEEL_DELTA / scroll_lines) as i32;
        } else {
            DELTA_PER_LINE = 0;
        }
        println!("DELTA_PER_LINE={}", DELTA_PER_LINE);
    }
}
        ...
        match message {
            ...
            WM_KEYDOWN => {
                match wparam as u16 {
                    ...
                    VK_PRIOR => { SendMessageW(window, WM_VSCROLL, SB_PAGEUP as usize, 0); },
                    VK_NEXT => { SendMessageW(window, WM_VSCROLL, SB_PAGEDOWN as usize, 0); },
                    VK_UP => { SendMessageW(window, WM_VSCROLL, SB_LINEUP as usize, 0); },
                    VK_DOWN => { SendMessageW(window, WM_VSCROLL, SB_LINEDOWN as usize, 0); },
                    VK_LEFT => { SendMessageW(window, WM_HSCROLL, SB_PAGEUP as usize, 0); },
                    VK_RIGHT => { SendMessageW(window, WM_HSCROLL, SB_PAGEDOWN as usize, 0); },
                    _ => {},
                }
                0
            },
            WM_MOUSEWHEEL => {
                ...
                let delta = hiword!(wparam) as i16;
                ACCUM_DELTA += delta as i32;
                while ACCUM_DELTA >= DELTA_PER_LINE {
                    SendMessageW(window, WM_VSCROLL, SB_LINEUP as usize, 0);
                    ACCUM_DELTA -= DELTA_PER_LINE;
                }

                while ACCUM_DELTA <= -DELTA_PER_LINE {
                    SendMessageW(window, WM_VSCROLL, SB_LINEDOWN as usize, 0);
                    ACCUM_DELTA += DELTA_PER_LINE;
                }
                0
            }
