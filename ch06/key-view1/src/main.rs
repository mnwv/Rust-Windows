...
fn main() {
    unsafe {
        ...
        let hwnd = CreateWindowExW(
            0,
            app_name,
            w!("This program requires Windows NT!"),
...
unsafe fn disp_key_msg(hwnd: HWND, message: u32, wparam: WPARAM, lparam: LPARAM) {
    let msg: MSG = MSG { hwnd: hwnd, message: message, wParam: wparam, lParam: lparam, time: 0, pt: POINT{ x:0, y:0}};
    PMSG.push(msg);
    while PMSG.len() as i32 > LINES_MAX {
        PMSG.remove(0);
    }
    let rect: RECT = RECT{left:0, right:CX_CLIENT, top: CY_CHAR, bottom: CY_CHAR * (CY_CLIENT / CY_CHAR),};
